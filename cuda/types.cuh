...
inline __host__ __device__ float len_sq3(const float3 &v) { return dot3(v, v); }
...
inline __host__ __device__ float3 refract3(const float3 &uv, const float3 &n,
                                           float eta_ratio) {
...
struct GpuScene {
  ...
  int bvh_node_count;
  const uint32_t
      *bvh_primitive_indices; // For leaf nodes: indices of primitives (spheres)
...
inline __host__ __device__ void set_face_normal(Hit &h, const Ray &r,
