...
void write_png(std::vector<color> framebuffer, int num_pixels, int width, int height, std::string output_name){
...
class camera {
    public:
        ...
        void render_parallel(const hittable&  world){
            ...
            std::vector<color> framebuffer(num_pixels);
            ...
            for (int j = 0; j < int(image_height); j++){
                for (int i = 0; i < int(image_width); i++){
                    ...
                    for (int k = 0; k < samples_per_pixel; k++){
                        ray r  = get_ray(i, j);
                        pixel_color += ray_color(r, max_depth, world); //color(1.0, 0.0, 0.0); //
                    }
            ...
            write_png(framebuffer, num_pixels, image_width, image_height, output_name);  
            auto t3 = omp_get_wtime();
           std::clog << "\rDone.                     \n"; 
           std::clog << "\rParallel(" << omp_get_max_threads() << ")" << " time:" << t2-t1 << "\n";
           std::clog << "\rParallel(" << omp_get_max_threads() << ") writing" << " time:" << t3-t2 << "\n";
        ...
        /* Render the image */
            ...
            std::string output_name = "complex_image.png";
            write_png(framebuffer, num_pixels, image_width, image_height, output_name);
            std::clog << "\r Sequencial time:" << t2-t1 << "\n";                 
            std::clog<< "\rDone.                 \n";
            }
