...
__global__ void init_rays(int samples_per_pixel, int width, int height, camera* cam, color* throughput, rayBuffer* rays, curandStatePhilox4_32_10_t* states){
    // Init all the rays
    ...
    for (int n = 0; n < samples_per_pixel; n++){
        ...
        cam->make_ray(i, j, idx, r_idx, rays, &state);    // Make and store rays in ray buffer and ray index in hit buffer
...
/*
__global__ void shade(hitBuffer* hits, rayBuffer* rays, color* throughput,
                      color* framebuffer, material** materials,
                      int n_rays, curandStatePhilox4_32_10_t* states) 
    {
    // Should be ray indexes for contiguous access in hittable
    int idx = threadIdx.x + blockIdx.x * blockDim.x;
    ...
    material* mat = materials[hits->mat_id[idx]];
    ...
    if (mat->scatter(hits, rays, idx, attenuation, state)) {
        throughput[idx] *= attenuation;
    } else {
        //framebuffer[p_idx] += color(0.0f, 0.0f, 0.0f);
        atomicAdd(&framebuffer[p_idx].e[0], 0.0f);
        atomicAdd(&framebuffer[p_idx].e[1], 0.0f);
        atomicAdd(&framebuffer[p_idx].e[2], 0.0f);
        rays->alive[idx] = 0;
    }
}
*/
...
__global__ void shade_lambertian(MaterialQueue* queue, hitBuffer* hits,
                                  ...
                                  material** materials, curandStatePhilox4_32_10_t* states) {
...
__host__ void initialize_states(int num_rays, unsigned long seed, curandStatePhilox4_32_10_t* states){
...
__host__ void initialize_rays(int samples_per_pixel, int width, int height,
                            camera* cam, color* throughput, rayBuffer* rays, curandStatePhilox4_32_10_t* states){
...
__host__ void render(
    ...
    color* framebuffer, int n_active, int max_depth, camera* cam, curandStatePhilox4_32_10_t* states){
...
int main(){
    ...
    for (int j=0; j<4; j++){
        ...
        CUDA_CHECK(cudaMalloc(&d_states, num_rays * sizeof(curandStatePhilox4_32_10_t)));
