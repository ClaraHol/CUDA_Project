...
class camera {
    public:
        ...
        float aspect_ratio = 1.0f;          // Ratio between width and height
        ...
        __device__ color ray_color(const ray& r, hittable** objects,  bvh_array* bvh, curandState* state) const {
            ...
            for (int i = 0; i< max_depth; i++){
                ...
                if (bvh[0].hit(bvh, objects, r_cur, interval(0.001, infinity), rec)) {
                    if (rec.mat->type == EMISSIVE) {
                        diffuse_light* light = (diffuse_light*)rec.mat;
                        //printf("Light: %d \n", throughput * light->emitted());
                        return throughput * light->emitted();
                    }
                    ray scattered;
                    color attenuation;
    ...
    // Private camera parameters
        ...
        float pixel_sample_scale;  // Color scale factor for sum of pixel samples
        ...
        __host__ void initialize(){
            ...
            pixel_sample_scale = 1.0f/samples_per_pixel;
