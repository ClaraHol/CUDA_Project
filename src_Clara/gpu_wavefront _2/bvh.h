...
/*
    ...
    runs on one thread. (Yes it is stupid, but I don't have time to implement a parallel tree building algorithm)
*/
...
class bvh_array {
    ...
    public:
        ...
        __device__ bool hit(bvh_array* nodes, hittable** objects, interval ray_t, hitBuffer* hits, rayBuffer* rays, int idx) const {
            ...
            while (stack_top > 0){
                ...
                if (!node.bbox.hit(rays, idx, ray_t)) continue;

                if (node.object_index >= 0){
                    if (objects[node.object_index]->hit(hits, rays, idx, ray_t)){
                        hit_anything = true;
                        ray_t.max = hits->t[idx];
                    } 
                } else {
            ...
            hits->hit_anything[idx] = hit_anything;
