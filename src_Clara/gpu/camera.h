...
class camera {
    public:
        ...
        __device__ color ray_color_iterative(const ray& r,  hittable** world, curandState* state) const {
           ...
           for (int i = 0; i < max_depth; i++){
                ...
                if ((*world) -> hit(r_cur, interval(0.001, infinity), rec)) {
                    
                    if (rec.mat->type == EMISSIVE) {
                        diffuse_light* light = (diffuse_light*)rec.mat;
                        color output = throughput * light->emitted();
                        return output;
                    }
                    
                    ray scattered;
                    color attenuation;
                    if (rec.mat -> scatter(r_cur, rec, attenuation, scattered, state)){
                    ...
                    vec3 unit_direction = unit_vector(r_cur.direction());
        ...
        __device__ color ray_color_recursive(const ray& r, int depth, hittable** world, curandState* state) const {
            ...
            if ((*world) -> hit(r, interval(0.001, infinity), rec)){
                ...
                if (rec.mat -> scatter(r, rec, attenuation, scattered, state))
                    return attenuation * ray_color_recursive(scattered, depth - 1, world, state);

                vec3 direction = rec.normal + random_unit_vector(state);
                return 0.5 * ray_color_recursive(ray(rec.p, direction), depth - 1,  world, state);
            }
