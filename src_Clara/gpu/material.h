...
class metal : public material {
    public: 
        __device__ metal(const color& albedo, double fuzz) : material(METAL), albedo(albedo), fuzz(fuzz < 1 ? fuzz: 1) {}
...
class dielectric : public material {
    ...
    public:
        __host__ __device__ dielectric(double refraction_index) : material(DIELECTRIC), refraction_index(refraction_index) {}
        ...
        __device__ bool scatter(const ray& r_in, const hit_record& rec, color& attenuation, ray& scattered, curandState* state) const override {
            ...
            if (cannot_refrac || reflectance(cos_theta, ri) > random_double(state))
