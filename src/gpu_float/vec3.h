...
class vec3 {
    ...
    public:
        ...
        __device__ static vec3 lcg_random_double(uint32_t& state){
        ...
        __device__ static vec3 lcg_random_double(double min, double max, uint32_t& state){
...
__device__ inline vec3 random_on_unit_disk(curandState* state){
    ...
    */
    ...
    auto r = sqrtf(random_double(state));         // Sample radius
    auto a = random_double(0.0, 2*pi, state);    // Sample angle

    vec3 p = vec3(r*cosf(a), r*sinf(a), 0.0);     // Convert to polar coordinates
    return p;
}
